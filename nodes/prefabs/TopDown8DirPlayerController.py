"""
TopDown 8-Direction Player Controller for Lupine Engine
A kinematic body player controller for top-down games with 8-directional movement
"""
...
class TopDown8DirPlayerController(KinematicBody2D):
    """
    TopDown 8-Direction Player Controller: A kinematic body for top-down games.
    ...
    def __init__(self, name: str = "TopDown8DirPlayerController"):
        super().__init__(name)
        self.type = "TopDown8DirPlayerController"
        ...
        self.export_variables.update({
            ...
            "normalize_diagonal": {
                "type": "bool",
                "value": True,
                "description": "Normalize diagonal movement for consistent speed"
            }
        })
        ...
        self.normalize_diagonal: bool = True
    ...
    def _ready(self):
        ...
        super()._ready()
        print(f"[DEBUG] TopDown8DirPlayerController._ready() called for {self.name}")
        ...
        try:
            ...
            if game_engine and hasattr(game_engine, 'is_action_pressed'):
                ...
                print(f"[DEBUG] TopDown8DirPlayerController: Found is_action_pressed in game engine")
            else:
                print(f"[TopDown8DirPlayerController] Warning: 'is_action_pressed' not found in game engine for {self.name}")
        except Exception as e:
            print(f"[TopDown8DirPlayerController] Error getting 'is_action_pressed' from game engine: {e}")
        ...
        try:
            ...
            if self._is_action_pressed_func:
                ...
                if self._is_action_pressed_func("move_right"):
                    input_x += 1.0
                if self._is_action_pressed_func("move_up"):
                    input_y += 1.0
                if self._is_action_pressed_func("move_down"):
                    input_y -= 1.0
            else:
            ...
            if self.normalize_diagonal and input_x != 0.0 and input_y != 0.0:
                # Normalize to maintain consistent speed in all directions
                length = math.sqrt(input_x * input_x + input_y * input_y)
                if length > 0:
                    input_x /= length
                    input_y /= length
            ...
            print(f"[TopDown8DirPlayerController] Input error: {e}")
    ...
    def get_movement_direction(self) -> int:
        """Get current movement direction as integer (0-7 for 8 directions, -1 for no movement)"""
        if abs(self._input_vector[0]) < 0.1 and abs(self._input_vector[1]) < 0.1:
            return -1

        # Calculate angle and convert to 8-direction index
        angle = math.atan2(self._input_vector[1], self._input_vector[0])
        # Convert to degrees and normalize to 0-360
        degrees = (math.degrees(angle) + 360) % 360
        # Convert to 8-direction index (0=right, 1=down-right, 2=down, etc.)
        direction = int((degrees + 22.5) / 45) % 8
        return direction

    def to_dict(self) -> Dict[str, Any]:
        """Serialize TopDown8DirPlayerController to a dictionary"""
        data = super().to_dict()
        data.update({
            "speed": self.speed,
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "TopDown8DirPlayerController":
        """Deserialize a TopDown8DirPlayerController from a dictionary"""
        controller = cls(data.get("name", "TopDown8DirPlayerController"))
        ...
        # Apply TopDown8DirPlayerController specific properties
        ...
        controller.normalize_diagonal = data.get("normalize_diagonal", True)
